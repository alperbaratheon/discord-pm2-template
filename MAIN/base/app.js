...
const eventsPath = path.join(__dirname, "../apps/Moderation/events");
const settings  = require("./globalsettings.json");
const commandsPath = path.join(__dirname, "../apps/Moderation/commands");
...
const rest = new REST({ version: '9' }).setToken(settings.tokens.Executive)
;

setTimeout(async () => {
    rest.put(Routes.applicationCommands(settings.guild.slashcommandclientid), { body: commands }).catch(console.error);
}, 500);